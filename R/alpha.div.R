...
alpha.div <- function(x, sample.col = 1) {
  x %>%
    select(sample.col) %>%
    mutate(Shannon = vegan::diversity(x[,2:length(x)], index = 'shannon'),
           Simpson = vegan::diversity(x[,2:length(x)], index = 'simpson'),
           Pielou = pielou(x[,2:length(x)]) ,
           Richness = vegan::specnumber(x[,2:length(x)])) %>%
